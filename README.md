...
系统通过以下字段组合判断零件唯一性，防止重复录入：

| 校验字段 | 说明 | 示例 |
|---------|------|------|
| **名称** (name) | 零件型号名称 | SS54、ESP-12F |
| **制造商** (manufacturer) | 生产厂商 | TI、安信可 |
| **封装** (package) | 物理封装类型 | SOT-23、DIP-8 |

**入库流程：**
```
输入数据 (名称 + 制造商 + 封装 + 数量 + ...)
    ↓
查询库存中是否存在相同的 "名称 + 制造商 + 封装" 组合
    ↓
存在 → 直接增加该零件的库存数量
不存在 → 创建新零件记录 + 创建库存记录
```
...
**注意事项：**
- 同一零件使用不同名称（如 `SS54` vs `SS-54`）会被视为不同零件
- 同一制造商使用不同写法（如 `TI` vs `Texas Instruments`）会被视为不同制造商
- LC Number 不参与去重判断，仅作为参考信息存储
